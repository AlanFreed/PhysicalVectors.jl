...
function isDimensionless(t::PhysicalTensor)::Bool
    return PhysicalSystemsOfUnits.isDimensionless(t.u)
end

function isDimensionless(at::ArrayOfPhysicalTensors)::Bool
    return PhysicalSystemsOfUnits.isDimensionless(at.u)
end

function isCGS(t::PhysicalTensor)::Bool
    return PhysicalSystemsOfUnits.isCGS(t.u)
end

function isCGS(at::ArrayOfPhysicalTensors)::Bool
    return PhysicalSystemsOfUnits.isCGS(at.u)
end

function isSI(t::PhysicalTensor)::Bool
    return PhysicalSystemsOfUnits.isSI(t.u)
end

function isSI(at::ArrayOfPhysicalTensors)::Bool
    return PhysicalSystemsOfUnits.isSI(at.u)
end
...
function toCGS(t::PhysicalTensor)::PhysicalTensor
    if isCGS(t)
        return t
    elseif isSI(t)
        units = PhysicalSystemsOfUnits.CGS(t.u.m, t.u.kg, t.u.s, t.u.K)
        tensor = newPhysicalTensor(t.r, t.c, units)
        if (t.u == PhysicalSystemsOfUnits.KELVIN)
            for i in 1:t.r
                for j in 1:t.c
...
function toCGS(at::ArrayOfPhysicalTensors)::ArrayOfPhysicalTensors
    if isCGS(at)
        return at
    elseif isSI(at)
        units = PhysicalSystemsOfUnits.CGS(at.u.m, at.u.kg, at.u.s, at.u.K)
        pt₁ = newPhysicalTensor(at.r, at.c, units)
        if (at.u == PhysicalSystemsOfUnits.KELVIN)
            for i in 1:at.r
                for j in 1:at.c
...
function toSI(t::PhysicalTensor)::PhysicalTensor
    if isSI(t)
        return t
    elseif isCGS(t)
        units = PhysicalSystemsOfUnits.SI(t.u.cm, t.u.g, t.u.s, t.u.C)
        tensor = newPhysicalTensor(t.r, t.c, units)
        if (t.u == PhysicalSystemsOfUnits.CENTIGRADE)
            for i in 1:t.r
                for j in 1:t.c
                    tensor.m[i,j] = 273.15 + t.m[i,j]
                end
...
function toSI(at::ArrayOfPhysicalTensors)::ArrayOfPhysicalTensors
    if isSI(at)
        return at
    elseif isCGS(at)
        units = PhysicalSystemsOfUnits.SI(at.u.cm, at.u.g, at.u.s, at.u.C)
        pt₁ = newPhysicalTensor(at.r, at.c, units)
        if (at.u == PhysicalSystemsOfUnits.CENTIGRADE)
            for i in 1:at.r
                for j in 1:at.c
                    pt₁.m[i,j] = 273.15 + at.a[1,i,j]
                end
...
function Base.:(copy)(y::PhysicalTensor)::PhysicalTensor
    ...
    matrix  = copy(y.m)
    units   = copy(y.u)
    return PhysicalTensor(UInt16(rows), UInt16(columns), matrix, units)
end
...
function Base.:(copy)(y::ArrayOfPhysicalTensors)::ArrayOfPhysicalTensors
    ...
    array   = copy(y.a)
    units   = copy(y.u)
    return ArrayOfPhysicalTensors(UInt16(entries), UInt16(rows), UInt16(columns), array, units)
end
...
function Base.:*(y::PhysicalTensor, z::PhysicalVector)::PhysicalVector
    ...
    if ((isCGS(y) && PhysicalVectors.isCGS(z)) || (isSI(y) && PhysicalVectors.isSI(z)))
        units = y.u + z.u
    ...
    elseif (isCGS(y) && PhysicalVectors.isSI(z))
        w = PhysicalVectors.toCGS(z)
        units = y.u + w.u
        vector = newPhysicalVector(y.r, units)
        for i in 1:y.r
            scalar = newPhysicalScalar(units)
    ...
    elseif (isSI(y) && PhysicalVectors.isCGS(z))
        w = toCGS(y)
        units = w.u + z.u
...
function Base.:*(y::PhysicalScalar, z::PhysicalTensor)::PhysicalTensor
    if ((PhysicalScalars.isCGS(y) && isCGS(z)) ||
        (PhysicalScalars.isSI(y) && isSI(z)))
        units = y.u + z.u
...
function Base.:/(y::PhysicalTensor, z::PhysicalScalar)::PhysicalTensor
    if ((isCGS(y) && PhysicalScalars.isCGS(z)) ||
        (isSI(y) && PhysicalScalars.isSI(z)))
        units = y.u - z.u
...
function Base.:\(A::PhysicalTensor, b::PhysicalVector)::PhysicalVector
    ...
    if ((isCGS(A) && PhysicalVectors.isCGS(b)) ||
        (isSI(A) && PhysicalVectors.isSI(b)))
        units = b.u - A.u
        vector = A.m \ b.v
    elseif isCGS(A) && isSI(b)
        w = PhysicalVectors.toCGS(b)
        units = w.u - A.u
        vector = A.m \ w.v
...
function tensorProduct(y::PhysicalVector, z::PhysicalVector)::PhysicalTensor
    if ((PhysicalVectors.isCGS(y) && PhysicalVectors.isCGS(z)) ||
        (PhysicalVectors.isSI(y) && PhysicalVectors.isSI(z)))
        units = y.u + z.u
    ...
    elseif (PhysicalVectors.isCGS(y) && PhysicalVectors.isSI(z))
        w = PhysicalVectors.toCGS(z)
        units = y.u + w.u
    ...
    elseif (PhysicalVectors.isSI(y) && PhysicalVectors.isCGS(z))
        w = PhysicalVectors.toCGS(y)
        units = w.u + z.u
